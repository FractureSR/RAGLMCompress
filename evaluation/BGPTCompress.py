...
class CompressionConfig:
    ...
    # Dataset paths
    DATASET_IMAGE = "datasets/clic2024/bmp/*.bmp"
    DATASET_AUDIO = "datasets/peoples_speech/wav_8k/*.wav"
    ...
    DATA_TYPE = "audio" # or image, used in test_worklow
    
    # BMP splitting parameters
    BMP_PATCH_SIZE = 32  # Size of square patches for BMP splitting

    # Audio splitting parameters
    AUDIO_CHUNK_DURATION = 0.5  # Duration of each audio chunk in seconds
...
def bgpt_compress(compress_input, logits, metric, precision=None, prefix_length=None):
    ...
    compress_ratio, compress_rate = metric.compute_ratio()
...
def load_dataset(dataset_path, device, max_files=None) -> List[Tuple[dict, List[int]]]:
    """
    Load dataset for compression testing using full files.
    :param dataset_path: glob pattern for dataset files
    :param device: torch device
    :param max_files: max number of files to read, None means all
...
def load_workflow_dataset(
    ...
    max_files=None,
    max_segments=None,
    temp_folder="temp_workflow",
):
    """
    Load dataset for workflow testing.
    Files are first split into small segments using existing modality-specific methods:
    - image: split_bmp_to_patches
    - audio: split_wav_to_chunks

    :param dataset_path: glob pattern for dataset files
    :param device: torch device
    :param data_type: "image" or "audio"
    :param max_files: max number of input files
    :param max_segments: max number of test segments
    :param temp_folder: temporary folder for split results
    :return: list of tuples (padded_segment, ext)
    """
    print("Loading workflow dataset for compression testing...")
    ...
    if data_type == "image":
        split_folder = os.path.join(temp_folder, "split_bmp")
        selected_input_folder = os.path.join(temp_folder, "selected_bmp")

        if os.path.exists(split_folder):
            shutil.rmtree(split_folder)
        if os.path.exists(selected_input_folder):
            shutil.rmtree(selected_input_folder)

        os.makedirs(split_folder, exist_ok=True)
        os.makedirs(selected_input_folder, exist_ok=True)

        # 只复制 MAX_FILES 选中的图片
        for af in fs:
            shutil.copy2(af, os.path.join(selected_input_folder, os.path.basename(af)))

        # 只对选中的图片进行切片
        split_bmp_to_patches(
            source_folder=selected_input_folder,
            output_folder=split_folder,
            patch_size=CompressionConfig.BMP_PATCH_SIZE
        )

        # 读取切片结果
        for af in tqdm(fs, total=len(fs)):
            ...
            for patch_file in patch_files:
                ...
                if max_segments is not None and len(dataset) >= max_segments:
                    print(f"Loaded {len(dataset)} segments for workflow testing.")
                    return dataset

    elif data_type == "audio":
        split_folder = os.path.join(temp_folder, "split_wav")
        os.makedirs(split_folder, exist_ok=True)

        for af in tqdm(fs, total=len(fs)):
            chunk_files, _ = split_wav_to_chunks(
                af,
                split_folder,
...
def test_bmp_compression(
    ...
    """
    Test BMP file compression and optional decompression workflow.
    
    This function:
    1. Splits selected BMP files into small patches
    2. Compresses each patch separately
    3. Optionally decompresses each patch
    4. Optionally merges patches back into the original image
    5. Optionally verifies the reconstructed image matches the original
    ...
    :param do_decompress: whether to perform decompression and reconstruction
    """
    if patch_size is None:
        patch_size = CompressionConfig.BMP_PATCH_SIZE
    
    # Temporarily switch to image mode to get correct paths
    ...
    if CompressionConfig.MAX_FILES is not None:
        bmp_files = bmp_files[:CompressionConfig.MAX_FILES]
    ...
    for bmp_file in bmp_files:
        shutil.copy2(bmp_file, os.path.join(selected_input_folder, os.path.basename(bmp_file)))
    ...
    for bmp_file in bmp_files:
        ...
        # Step 1: Use pre-split patches
        print(f"Step 1: Using pre-split {patch_size}x{patch_size} patches...")
        ...
        if do_decompress:
            ...
            print("\nStep 3~5 skipped because do_decompress=False")
    ...
    if do_decompress:
        ...
        print("Decompression was skipped; no reconstructed images were generated.")
    print("Compression ratio/rate:", total_metric.compute_ratio())
...
def test_audio_compression(
    ...
    """
    Test audio file (WAV) compression and optional decompression workflow.
    
    This function:
    1. Splits each WAV file into chunks (e.g., 1 second each)
    2. Compresses each chunk separately
    3. Optionally decompresses each chunk
    4. Optionally merges chunks back into the original audio
    5. Optionally verifies the reconstructed audio matches the original
    ...
    if CompressionConfig.MAX_FILES is not None:
        wav_files = wav_files[:CompressionConfig.MAX_FILES]
    ...
    if do_decompress:
        ...
        print("Decompression was skipped; no reconstructed audio files were generated.")
    print("Compression ratio/rate:", total_metric.compute_ratio())
...
if __name__ == "__main__":
    # =====================================================
    # Choose mode:
    #   1) "workflow"   : standard workflow test
    #   2) "bmp"        : BMP compression test
    #   3) "audio"      : audio compression test
    # =====================================================
    parser = argparse.ArgumentParser(description="bGPT compression test entry")
    parser.add_argument(
        "--mode",
        type=str,
        default="bmp",
        choices=["workflow", "bmp", "audio"],
        help='Run mode: "workflow", "bmp", or "audio"'
    )
    args = parser.parse_args()
    MODE = args.mode   # 可选: "workflow", "bmp", "audio"

    # Setup device
    device = torch.device(CompressionConfig.DEVICE)

    if MODE in ["workflow", "bmp"]:
        CompressionConfig.DATA_TYPE = "image"
    elif MODE == "audio":
        CompressionConfig.DATA_TYPE = "audio"
    else:
    ...
    if MODE == "workflow":
        ...
        dataset = load_workflow_dataset(
            ...
            device=device,
            data_type=CompressionConfig.DATA_TYPE,
            max_files=CompressionConfig.MAX_FILES,
            max_segments=CompressionConfig.MAX_TEST_SEGMENTS,
            temp_folder="temp_workflow",
        )

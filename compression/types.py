...
class LMScore:
    ...
    caller can rank prompt contexts by ideal model code length and inspect where
    the model is still surprised — without paying for the range coder. ``bits``
    is ``sum(token_nll) / ln 2``; ``token_nll`` is the
